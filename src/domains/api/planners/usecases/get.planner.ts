...
import { FindPlannerCriteria, GetPlannerInteractorDependencies, InputGetPlanner } from "../interfaces";
...
export class GetPlannerInteractor {
  ...
  async execute(input: InputGetPlanner): Promise<HttpResponse> {
    try {      
      this.gateway.loggerInfo('Iniciando a busca dos planejamentos anuais', { requestTxt: JSON.stringify(input) });
      const { id_user, limite, id_company } = input;
           
      const user = await this.gateway.findUser({ id: id_user })
      if(user && user.id_company !== id_company) {
        this.gateway.loggerInfo('Usuário não pertence a empresa informada', { id_user: id_user, id_company: id_company });
        return this.presenter.forbidden('Usuário não pertence a empresa informada');
      }
      
      const criteria: FindPlannerCriteria = { id_company, limite };
